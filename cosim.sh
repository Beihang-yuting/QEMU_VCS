...
test_guide_info() {
    ...
    echo ""
    echo "  注意事项:"
    echo "    - 仿真速度较慢，每个 TLP 往返约数秒"
    echo "    - ping/arping 超时建议设为 600 秒以上"
    echo "    - iperf 测试需要 Guest 和 VCS 机器都安装 iperf3"
    echo "    - Guest 中的 nc 可能不可用，建议用 ping/arping 做连通性测试"
    echo "    - VCS 机器的 nc 实际是 ncat，-w 参数单位是秒"
}
...
test_guide_iperf() {
    ...
    echo ""
    echo "  前置条件:"
    echo "    - Guest rootfs 包含 iperf3（buildroot menuconfig 启用）"
    echo "    - VCS 机器（TAP 侧）也需安装 iperf3: sudo apt install iperf3 或 yum install iperf3"
    echo "    - 建议先通过 ping 测试确认连通性"
    echo ""
    ...
    echo ""
    echo "    # TCP 测试"
    echo "    iperf3 -c 10.0.0.1 -p 5201 -t 10 --connect-timeout 60000"
    echo ""
    echo "    # UDP 测试"
    echo "    iperf3 -c 10.0.0.1 -p 5201 -u -b 1M -t 10 --connect-timeout 60000"
    echo ""
    echo "  注意:"
    echo "    - 仿真速度限制，实际吞吐量远低于真实网络"
    echo "    - TCP 三次握手通过 cosim 仿真可能需要数分钟"
    echo "    - --connect-timeout 60000 设为 60 秒避免连接超时"
    echo "    - 如 VCS 机器无 iperf3，可用 nc 替代:"
    echo "      VCS 侧: ncat -l -p 9999 --idle-timeout 600s"
    echo "      Guest:  echo TEST | nc -w 120 10.0.0.1 9999"
}
