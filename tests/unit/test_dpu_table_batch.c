                          ...
                          int order)
{
    ...
    CHECK((uint64_t)entry_bytes * entry_count <= sizeof(payload));
    fill_payload(payload, (size_t)entry_bytes * entry_count);
    CHECK(dpu_table_batch_core(&fixture, 0x1000, payload, entry_bytes,
                                stride_bytes, entry_count, order, slot_bytes,
                                submit_once, NULL) ==
           DPU_TABLE_BATCH_CORE_SUCCESS);
    CHECK(fixture.call_count ==
    ...
    for (call = 0; call < fixture.call_count; ++call) {
        ...
        CHECK(fixture.calls[call].offset ==
               0x1000 + (uint64_t)submitted * stride_bytes);
        CHECK(fixture.calls[call].data == payload +
               (size_t)submitted * entry_bytes);
        CHECK(fixture.calls[call].payload_bytes == bytes);
        CHECK(fixture.calls[call].payload_bytes % entry_bytes == 0);
        CHECK(fixture.calls[call].order == order);
        CHECK(memcmp(fixture.calls[call].data,
                      payload + (size_t)submitted * entry_bytes,
                      bytes) == 0);
        submitted += entries;
    }
    CHECK(submitted == entry_count);
}
...
static void test_payload_larger_than_slot_uses_multiple_requests(void)
{
    ...
    CHECK(dpu_table_batch_core(
               &fixture, 0x8000, payload, 128, 192, 17,
               DPU_TABLE_BATCH_CORE_LOW_TO_HIGH, 512, submit_once, NULL) ==
           DPU_TABLE_BATCH_CORE_SUCCESS);
    CHECK(fixture.call_count == 5);
    for (call = 0; call < 4; ++call)
        CHECK(fixture.calls[call].payload_bytes == 512);
    CHECK(fixture.calls[4].payload_bytes == 128);
    CHECK(fixture.calls[4].offset == 0x8000 + 16u * 192u);
    CHECK(fixture.calls[4].data == payload + 16u * 128u);
}
...
static void test_hard_and_partial_frontdoor_errors_report_progress(void)
{
    ...
    CHECK(progress.committed_entries == 2);
    CHECK(progress.failed_index == 2);
    CHECK(progress.failed_offset == 0x1040);
    CHECK(progress.original_result == DPU_TABLE_BATCH_CORE_ERROR);
    CHECK(progress.final_result == DPU_TABLE_BATCH_CORE_ERROR);
    ...
    CHECK(progress.committed_entries == 2);
    CHECK(progress.failed_index == 2);
    CHECK(progress.failed_offset == 0x2040);
    CHECK(progress.original_result == DPU_TABLE_BATCH_CORE_FRONTDOOR);
    CHECK(progress.final_result == DPU_TABLE_BATCH_CORE_ERROR);
}
...
static void test_invalid_and_overflow_inputs_do_not_submit(void)
{
    ...
    CHECK(dpu_table_batch_core(&fixture, 0, payload, 64, 64, 1,
                                DPU_TABLE_BATCH_CORE_LOW_TO_HIGH, 63,
                                submit_once, NULL) ==
           DPU_TABLE_BATCH_CORE_FRONTDOOR);
    CHECK(dpu_table_batch_core(&fixture, UINT64_MAX - 15, payload,
                                16, 32, 2,
    ...
    CHECK(fixture.call_count == 1);
    memset(&fixture, 0, sizeof(fixture));
    CHECK(dpu_table_batch_core(&fixture, UINT64_MAX - 14, payload,
                                16, 32, 1,
                                DPU_TABLE_BATCH_CORE_LOW_TO_HIGH, 64,
                                submit_once, NULL) ==
           DPU_TABLE_BATCH_CORE_FRONTDOOR);
    CHECK(dpu_table_batch_core(&fixture, UINT64_MAX, payload, UINT32_MAX,
                                UINT32_MAX, UINT32_MAX,
                                DPU_TABLE_BATCH_CORE_LOW_TO_HIGH, UINT32_MAX,
                                submit_once, NULL) ==
           DPU_TABLE_BATCH_CORE_FRONTDOOR);
    CHECK(dpu_table_batch_core(&fixture, 0, payload, 16, 15, 1,
                                DPU_TABLE_BATCH_CORE_LOW_TO_HIGH, 64,
                                submit_once, NULL) ==
           DPU_TABLE_BATCH_CORE_FRONTDOOR);
    CHECK(dpu_table_batch_core(&fixture, 0, payload, 16, 16, 0,
                                DPU_TABLE_BATCH_CORE_LOW_TO_HIGH, 64,
                                submit_once, NULL) ==
           DPU_TABLE_BATCH_CORE_FRONTDOOR);
    CHECK(dpu_table_batch_core(&fixture, 0, payload, 16, 16, 1, 7, 64,
                                submit_once, NULL) ==
           DPU_TABLE_BATCH_CORE_FRONTDOOR);
