...
# 或跨机:
./setup.sh --mode qemu-only --guest minimal
```

编译产出: Bridge 库 + QEMU (含 cosim-pcie-rc) + Guest initramfs
...
| Buildroot | `logs/buildroot_build.log` | Guest 编译 |
...
| `eth0 not found` | 驱动未加载 | 检查 initramfs 含 virtio 驱动 |
