...
编译产出: Bridge 库 + QEMU (含 cosim-pcie-rc / cosim-pcie-pf) + Guest 根文件系统镜像

#### 驱动模式选择

setup.sh 支持 `--driver` 参数选择 PF 驱动模式:

```bash
# stub 模式（默认）— 内置 cosim_nic 驱动，快速验证 SR-IOV
./setup.sh --mode local --guest ubuntu --driver stub

# custom 模式 — 使用自定义驱动
./setup.sh --mode local --guest ubuntu --driver custom --ko /path/to/my_nic.ko

# none 模式 — 不加载驱动（手动操作）
./setup.sh --mode local --guest ubuntu --driver none
```
...
### 3.5 SR-IOV 多 Function 模式

```bash
# 终端 A: 启 QEMU (NUM_PFS>1 自动使用 cosim-pcie-pf)
...
# Guest 内:
insmod /lib/modules/cosim_nic.ko                            # 加载 stub 驱动
echo 4 > /sys/bus/pci/devices/0000:00:03.0/sriov_numvfs     # PF0 创建 4 VF
echo 4 > /sys/bus/pci/devices/0000:00:03.1/sriov_numvfs     # PF1 创建 4 VF
lspci | grep Vada                                            # 查看所有 PF + VF

# VF 间 iperf 测试（需要 namespace 隔离）
ip netns add ns1 && ip netns add ns2
ip link set eth4 netns ns1 && ip link set eth5 netns ns2
ip netns exec ns1 bash -c "ip addr add 10.0.1.1/24 dev eth4 && ip link set eth4 up"
ip netns exec ns2 bash -c "ip addr add 10.0.1.2/24 dev eth5 && ip link set eth5 up"
ip netns exec ns2 iperf3 -s -D
ip netns exec ns1 iperf3 -c 10.0.1.2 -t 5
...
| `NUM_PFS` | `1` | PF 数量 (>1 自动使用 cosim-pcie-pf) |
| `MAX_VFS` | `0` | 每 PF 最大 VF 数 |
| `MSIX_VECTORS` | `4` | PF MSI-X 向量数 |
| `VF_MSIX_VECS` | `2` | VF MSI-X 向量数 |
...
| Guest 构建 | `logs/guest_build.log` | Ubuntu/Debian Guest 构建 |
...
| `Kernel panic: Unable to mount root fs` | initramfs 未注入 cosim-init | 重新构建: `rm -rf guest/images/* && ./setup.sh --mode local --guest ubuntu` |
